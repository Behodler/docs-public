# PhUSD Backing
phUSD can be thought of as backed by two primary sources: the vault TVL (vault_TVL) and the AMM TVL (AMM_TVL).

While it may be intuitive that AMM TVL backs phUSD, it may not be as clear why vault TVL provides a backing.
...
However, when you consider the full picture of Phoenix, a remarkable degree of stability emerges. Instead of fixed redemption contracts, we have created a stable equilibrium, an attractor. Anything outside of the equilibrium is unstable and forced to collapse back into stability.

Consider the scenario where stakers suddenly unstake their phUSD and sell onto the AMM.

The immediate effect is that APY on the staking module sky rockets. Why is this? The majority of incentives in the staking module are comprised of externally sourced stablecoins. This means that the flow of value to stakers is independent of the current phUSD price. In other words, if the current total yield is $100 per month and the price of phUSD halves, it remains $100 per month.
...
Where would the phUSD used to drop the price further come from? The vast majority that is outside of the AMM is in the staking module. As users unstake, the staking APY rises and the available quantity of phUSD to sell dwindles. This creates a price floor combined with a massive staking APY. 
...
Adding to this instability, the paired token in the AMM is a yield-bearing stablecoin, meaning that the price floor is now growing at the rate of the yield-bearing stablecoin.

New stakers can either buy on the AMM or mint and stake. However, purchasing on the AMM will be much cheaper. And so the initial outflow from staking to the AMM collapses and is reversed.
...
So returning to the question: "is this stable?" On a block to block basis, it can be potentially more volatile than a 1:1 backing but the sheer gravitational force binding the $1 price in place means that what it loses in momentary predictability, it makes up for in long-term *singularity*.